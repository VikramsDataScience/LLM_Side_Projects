...
## Command line usage
In my zeal to make my code as clean & production ready as possible (i.e. using best practices from the Sofware Engineering world), I've changed the project structure to use a cleaner method of loading global variables and file paths through a `config.py` module, rather than the messier YAML file. Whilst this has made the code much cleaner, it also means that (at least in my local environment) that in the command line the modules have to be loaded from the source folder ('ECommerce_Churn_Propensity_Model'), and not the 'ECommerce_Churn_Propensity_Model/src' folder. In so doing, the typical command has to be modified to `python -m src.[modulename]`. For instance, if you're to run the PreProcessing.py module, you'd type `python -m src.PreProcessing` from the 'ECommerce_Churn_Propensity_Model' folder.
