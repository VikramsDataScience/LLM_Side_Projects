...
class FineTuneLRFinder(LightningModule):
    ...
    """
    def __init__(self, pretrained_model, pretrained_tokenizer, custom_data, learning_rate, batch_size):
        super().__init__()
    ...
    def forward(self, inputs, target):
        ...
        """
        return self.pretrained_model(inputs, target), self.custom_data(inputs, target)
    ...
    def training_step(self, batch, batch_idx):
        ...
        #self.log('train_loss', loss, on_step=True, on_epoch=True, prog_bar=True, logger=True, batch_size=batch)

        return loss

    def collate_fn(self, padding_value):
        """
        ...
        """
        padding_value = pad_sequence(self.custom_data, batch_first=True)

        return padding_value

    def train_dataloader(self):
        """
        - Divide training set into batches, pad shorter sentences using the defined 'collate_fn()'
        - 'pin_memory' arg allows for faster transfer of data to the GPU, and num_workers enables
        asynchronous data fetching that won't block GPU computation.
        """
    ...
    def configure_optimizers(self):
        learning_rate = float(self.learning_rate) if isinstance(self.learning_rate, str) else self.learning_rate
        
        return torch.optim.AdamW(self.parameters(), lr=learning_rate)

# Encode text corpus, split into train/test sets, convert train set to tensor, and mount to GPU
...
trainer.fit(model, train_dataloaders=train_tensor)
